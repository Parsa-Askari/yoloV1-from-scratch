...
def split_boxes(boxes,S,B,C):
    ...
    box1=boxes[...,21:25]
    box2=boxes[...,26:]
    best_prob,best=torch.max(torch.cat((boxes[...,20:21].unsqueeze(0),
                                        boxes[...,25:26].unsqueeze(0)),dim=0),dim=0)
...
@torch.no_grad()
def evaluation(dataloader,model,prob_thresh=0.4,iou_thresh=0.5,S=7,B=2,C=20,DEVICE="cpu"):
    model=model.eval()
